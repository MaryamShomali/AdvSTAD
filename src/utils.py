...
def plot_accuracies(accuracy_list, folder, loss_label='Average Training Loss'):
	os.makedirs(f'plots/{folder}/', exist_ok=True)
	trainAcc = [i[0] for i in accuracy_list]
	lrs = [i[1] for i in accuracy_list]
	plt.xlabel('Epochs')
	plt.ylabel(loss_label)
	plt.plot(range(len(trainAcc)), trainAcc, label=loss_label, linewidth=1, linestyle='-', marker='.')
	plt.twinx()
